...
// Auto migration for bookings table and tables status
try {
    ...
    // Ensure tables status enum includes 'booked'
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['add_booking'])) {
    try {
        ...
        $sql = "INSERT INTO bookings (customer_name, customer_phone, table_id, booking_date, booking_time, guests, note, status, created_by_role)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)";
        ...
        if ($table_id) {
            if ($status === 'pending' || $status === 'confirmed') {
                $pdo->prepare("UPDATE tables SET status = 'booked' WHERE id = ?")->execute([$table_id]);
            } elseif ($status === 'using') {
                $pdo->prepare("UPDATE tables SET status = 'busy' WHERE id = ?")->execute([$table_id]);
            }
        }
        $pdo->commit();
        setMessage('success', 'Đã thêm lịch đặt bàn mới!');
    } catch (Exception $e) {
        ...
        try {
            ...
            if ($booking && $booking['table_id']) {
                if ($action === 'confirm') {
                    ...
                    // Only free if it was booked/busy by THIS booking logic (roughly)
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['edit_booking'])) {
    try {
        ...
        $old_booking = fetchOne($pdo, "SELECT table_id, status FROM bookings WHERE id = ?", [$id]);
        $table_id = !empty($_POST['table_id']) ? $_POST['table_id'] : null;
        $new_status = $_POST['status'];
        $old_status = $old_booking['status'];
        
        $sql = "UPDATE bookings SET customer_name=?, customer_phone=?, table_id=?, booking_date=?, booking_time=?, guests=?, note=?, status=? WHERE id=?";
        $pdo->prepare($sql)->execute([
            $_POST['customer_name'],
            $_POST['customer_phone'],
            $table_id,
        ...
        if ($old_booking['table_id'] != $table_id || $old_status != $new_status) {
            // Free old table if it was booked
            if ($old_booking['table_id']) {
                $pdo->prepare("UPDATE tables SET status = 'free' WHERE id = ? AND status = 'booked'")->execute([$old_booking['table_id']]);
            }
            
            // Set new table to booked if booking is active
            if ($table_id && ($new_status === 'pending' || $new_status === 'confirmed')) {
                $pdo->prepare("UPDATE tables SET status = 'booked' WHERE id = ?")->execute([$table_id]);
            }
        }
        
        $pdo->commit();
        setMessage('success', 'Cập nhật đặt bàn thành công!');
    } catch (Exception $e) {
        if ($pdo->inTransaction()) $pdo->rollBack();
        setMessage('danger', 'Lỗi: ' . $e->getMessage());
    }
...
if (isset($_GET['action']) && $_GET['action'] === 'delete' && isset($_GET['id'])) {
    ...
    try {
        ...
        if ($booking) {
            ...
            if (($booking['status'] === 'pending' || $booking['status'] === 'confirmed') && $booking['table_id']) {
                $pdo->prepare("UPDATE tables SET status = 'free' WHERE id = ? AND status = 'booked'")->execute([$booking['table_id']]);
            }
        }
        $pdo->commit();
...
$sql = "SELECT b.*, t.name as table_name, c.full_name as linked_customer_name 
        FROM bookings b 
...
<div class="main-content-pos">
    ...
    <div class="d-flex justify-content-between align-items-center mb-4">
        <div class="d-flex align-items-center gap-3">
            ...
            <button class="btn btn-brown rounded-pill px-3 py-1-5 x-small fw-bold" data-bs-toggle="modal" data-bs-target="#addBookingModal">
                <i class="bi bi-plus-lg me-1"></i> Thêm đặt bàn
            </button>
        </div>
    </div>
    ...
    <?php if (empty($bookings)): ?>
        ...
        <div class="booking-empty-container shadow-sm border-0">
            <div class="empty-content text-center py-5">
                ...
                <button class="btn btn-brown rounded-pill px-4 py-2 fw-bold" data-bs-toggle="modal" data-bs-target="#addBookingModal">
                    <i class="bi bi-plus-lg me-2"></i> Thêm đặt bàn mới
                </button>
            </div>
        </div>
    <?php else: ?>
            ...
            ?>
                    ...
                    <div class="booking-card shadow-sm border-0">
                        <div class="card-header border-0 bg-white d-flex justify-content-between align-items-center pt-4 px-4 pb-0">
                            ...
                            <div class="d-flex align-items-center gap-2">
                                ...
                                <button class="btn btn-sm btn-icon rounded-circle bg-light border-0 shadow-sm" title="Chỉnh sửa" data-bs-toggle="modal" data-bs-target="#editBookingModal<?= $b['id'] ?>">
                ...
                <div class="modal fade" id="editBookingModal<?= $b['id'] ?>" tabindex="-1">
                    <div class="modal-dialog modal-dialog-centered">
                        <div class="modal-content rounded-24 border-0 shadow">
                            <div class="modal-header border-0 pt-4 px-4 overflow-hidden">
                                <h5 class="modal-title fw-bold">Sửa đặt bàn #BK-<?= $b['id'] ?></h5>
                            ...
                            <form method="POST">
                                ...
                                <div class="modal-body p-4 pt-2">
                                    <div class="row g-3 mb-3">
                                        <div class="col-md-6">
                                            <label class="form-label x-small fw-bold text-muted text-uppercase">Tên khách hàng</label>
                                            <input type="text" name="customer_name" class="form-control rounded-12 p-2-5" required value="<?= htmlspecialchars($b['customer_name']) ?>">
                                        </div>
                                        <div class="col-md-6">
                                            <label class="form-label x-small fw-bold text-muted text-uppercase">Số điện thoại</label>
                                            <input type="tel" name="customer_phone" class="form-control rounded-12 p-2-5" required value="<?= htmlspecialchars($b['customer_phone']) ?>">
                                        </div>
                                        <div class="col-md-6">
                                            <label class="form-label x-small fw-bold text-muted text-uppercase">Ngày đặt</label>
                                            <input type="date" name="booking_date" class="form-control rounded-12 p-2-5" required value="<?= $b['booking_date'] ?>">
                                        </div>
                                        <div class="col-md-6">
                                            <label class="form-label x-small fw-bold text-muted text-uppercase">Giờ đặt</label>
                                            <input type="time" name="booking_time" class="form-control rounded-12 p-2-5" required value="<?= date('H:i', strtotime($b['booking_time'])) ?>">
                                        </div>
                                        <div class="col-md-6">
                                            <label class="form-label x-small fw-bold text-muted text-uppercase">Số bàn (Chọn nếu có)</label>
                                            <select name="table_id" class="form-select rounded-12 p-2-5">
                                                <option value="">Đang xếp...</option>
                                                <?php foreach ($tables as $t): 
                                                    $is_current = ($b['table_id'] == $t['id']);
                                                    $status_label = '';
                                                    $status_class = '';
                                                    if ($t['status'] === 'booked') {
                                                        $status_label = ' - Đã đặt';
                                                        $status_class = 'text-info';
                                                    } elseif ($t['status'] === 'busy') {
                                                        $status_label = ' - Đang sử dụng';
                                                        $status_class = 'text-danger';
                                                    }
                                                ?>
                                                    <option value="<?= $t['id'] ?>" <?= $is_current ? 'selected' : '' ?> <?= ($t['status'] !== 'free' && !$is_current) ? 'class="bg-light"' : '' ?>>
                                                        <?= htmlspecialchars($t['name']) ?> (<?= $t['capacity'] ?> chỗ)<?= $status_label ?>
                                                    </option>
                                                <?php endforeach; ?>
                                            </select>
                                        </div>
                                        <div class="col-md-6">
                                            <label class="form-label x-small fw-bold text-muted text-uppercase">Số khách</label>
                                            <input type="number" name="guests" class="form-control rounded-12 p-2-5" value="<?= $b['guests'] ?>" min="1">
                                        </div>
                                        <div class="col-md-6">
                                            <label class="form-label x-small fw-bold text-muted text-uppercase">Trạng thái</label>
                                            <select name="status" class="form-select rounded-12 p-2-5">
                                                <option value="pending" <?= $b['status'] === 'pending' ? 'selected' : '' ?>>Chờ xác nhận</option>
                                                <option value="confirmed" <?= $b['status'] === 'confirmed' ? 'selected' : '' ?>>Đã xác nhận</option>
                                                <option value="using" <?= $b['status'] === 'using' ? 'selected' : '' ?>>Đang sử dụng</option>
                                                <option value="cancelled" <?= $b['status'] === 'cancelled' ? 'selected' : '' ?>>Đã hủy</option>
                                                <option value="completed" <?= $b['status'] === 'completed' ? 'selected' : '' ?>>Đã hoàn tất</option>
                                            </select>
                                        </div>
                                        <div class="col-12">
                                            <label class="form-label x-small fw-bold text-muted text-uppercase">Ghi chú</label>
                                            <textarea name="note" class="form-control rounded-12 p-3" rows="2"><?= htmlspecialchars($b['note'] ?? '') ?></textarea>
                                        </div>
                                    </div>
                                </div>
                                <div class="modal-footer border-0 p-4 pt-0 d-flex gap-2">
                                    <button type="button" class="btn btn-light rounded-pill px-4 flex-grow-1 fw-bold text-muted py-2-5" data-bs-dismiss="modal">Hủy bỏ</button>
                                    <button type="submit" name="edit_booking" class="btn btn-primary rounded-pill px-4 flex-grow-1 fw-bold py-2-5">Lưu thay đổi</button>
                                </div>
                            </form>
                        </div>
                    </div>
...
<div class="modal fade" id="addBookingModal" tabindex="-1">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content rounded-24 border-0 shadow">
            <div class="modal-header border-0 pt-4 px-4 overflow-hidden">
                <h5 class="modal-title fw-bold">Thông tin khách đặt bàn</h5>
                <button type="button" class="btn-close shadow-none" data-bs-dismiss="modal"></button>
            </div>
            <form method="POST">
                <div class="modal-body p-4 pt-2">
                    <div class="row g-3 mb-3">
                        <div class="col-md-6">
                            <label class="form-label x-small fw-bold text-muted text-uppercase">Tên khách hàng</label>
                            <input type="text" name="customer_name" class="form-control rounded-12 p-2-5" required placeholder="Nguyễn Văn A">
                        </div>
                        <div class="col-md-6">
                            <label class="form-label x-small fw-bold text-muted text-uppercase">Số điện thoại</label>
                            <input type="tel" name="customer_phone" class="form-control rounded-12 p-2-5" required placeholder="0123...">
                        </div>
                        <div class="col-md-6">
                            <label class="form-label x-small fw-bold text-muted text-uppercase">Ngày đặt</label>
                            <input type="date" name="booking_date" class="form-control rounded-12 p-2-5" required value="<?= $today ?>">
                        </div>
                        <div class="col-md-6">
                            <label class="form-label x-small fw-bold text-muted text-uppercase">Giờ đặt</label>
                            <input type="time" name="booking_time" class="form-control rounded-12 p-2-5" required value="19:00">
                        </div>
                        <div class="col-md-6">
                            <label class="form-label x-small fw-bold text-muted text-uppercase">Số bàn (Chọn nếu có)</label>
                            <select name="table_id" class="form-select rounded-12 p-2-5">
                                ...
                                ?>
                                    <option value="<?= $t['id'] ?>" <?= $t['status'] !== 'free' ? 'class="bg-light"' : '' ?>>
                                        <?= htmlspecialchars($t['name']) ?> (<?= $t['capacity'] ?> chỗ)<?= $status_label ?>
                                    </option>
                                <?php endforeach; ?>
                            </select>
                        ...
                        <div class="col-md-6">
                            <label class="form-label x-small fw-bold text-muted text-uppercase">Số khách</label>
                            <input type="number" name="guests" class="form-control rounded-12 p-2-5" value="2" min="1">
                        </div>
                        <div class="col-md-6">
                            <label class="form-label x-small fw-bold text-muted text-uppercase">Trạng thái</label>
                            <select name="status" class="form-select rounded-12 p-2-5">
                        ...
                        <div class="col-12">
                            <label class="form-label x-small fw-bold text-muted text-uppercase">Ghi chú</label>
                            <textarea name="note" class="form-control rounded-12 p-3" rows="2" placeholder="Yêu cầu đặc biệt nếu có..."></textarea>
                        </div>
                    </div>
                </div>
                <div class="modal-footer border-0 p-4 pt-0 d-flex gap-2">
                    <button type="button" class="btn btn-light rounded-pill px-4 flex-grow-1 fw-bold text-muted py-2-5" data-bs-dismiss="modal">Hủy bỏ</button>
                    <button type="submit" name="add_booking" class="btn btn-brown rounded-pill px-4 flex-grow-1 fw-bold py-2-5">Xác nhận đặt bàn</button>
                </div>
            </form>
        </div>
    </div>
