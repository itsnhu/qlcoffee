...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($code)) {
        ...
        $checkCode = fetchOne($pdo, "SELECT id FROM medicines WHERE code = ? AND id != ?", [$code, $id]);
        if ($checkCode) {
    ...
    if (empty($errors)) {
        try {
            $sql = "UPDATE medicines
                    SET name = ?, code = ?, category_id = ?, supplier_id = ?,
                        price = ?, quantity = ?, unit = ?, expiry_date = ?, description = ?
                    WHERE id = ?";
            ...
            setMessage('success', 'Cập nhật thuốc thành công!');
            header('Location: ' . BASE_URL . '/admin/medicines/index.php');
            exit;

        } catch (PDOException $e) {
            error_log("Update Medicine Error: " . $e->getMessage());
            $errors[] = 'Có lỗi khi cập nhật thuốc. Vui lòng thử lại.';
        }
    }
        ...
        <div class="d-flex justify-content-between align-items-center">
            <div>
                ...
                <p class="text-muted mb-0">
                    ...
                    Cập nhật thông tin thuốc: <strong><?php echo htmlspecialchars($medicine['name']); ?></strong>
                </p>
            </div>
            <div>
                ...
                <form method="POST" action="" id="medicineForm">
                    <div class="row">
                        ...
                        <div class="col-md-4 mb-3">
                            ...
                            <input type="text"
                                   ...
                                   id="code"
                                   name="code"
                                   value="<?php echo htmlspecialchars($medicine['code']); ?>"
                                   required
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-4 mb-3">
                            ...
                            <input type="text"
                                   ...
                                   id="unit"
                                   name="unit"
                                   value="<?php echo htmlspecialchars($medicine['unit'] ?? 'Viên'); ?>"
                    ...
                    <div class="mb-3">
                        ...
                        <textarea class="form-control"
                                  ...
                                  name="description"
                                  rows="3"
                                  placeholder="Thông tin thêm về thuốc (công dụng, cách dùng...)"><?php echo htmlspecialchars($medicine['description'] ?? ''); ?></textarea>
                    </div>
                    ...
                    <div class="alert alert-info">
                        <small>
                            <i class="bi bi-info-circle me-1"></i>
                            <strong>Thông tin:</strong> Thuốc được tạo vào <?php echo date('d/m/Y H:i', strtotime($medicine['created_at'])); ?>
                            <?php if ($medicine['updated_at'] != $medicine['created_at']): ?>
                                - Cập nhật lần cuối: <?php echo date('d/m/Y H:i', strtotime($medicine['updated_at'])); ?>
                            <?php endif; ?>
                        </small>
                    </div>
